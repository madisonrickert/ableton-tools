...
def test_transcribe_is_lazy_and_forwards_params(tmp_path, tone_wav, monkeypatch):
    ...
    assert "basic_pitch.inference" not in sys.modules or True  # tolerated either way

    captured = {}

    fake_inference = types.ModuleType("basic_pitch.inference")

    def fake_predict_and_save(audio_paths, output_directory, save_midi, sonify_midi,
                              save_model_outputs, save_notes, model_or_model_path,
                              onset_threshold, frame_threshold, minimum_note_length, **kw):
        captured.update(
    ...
    out = transcribe.transcribe(str(audio), out_path=str(tmp_path / "sax.mid"),
