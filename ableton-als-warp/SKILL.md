...
You warp and reposition `.als` clips using the shared Ableton engine
...
2. Dry-run:
   `<repo>/ableton-shared/bin/ableton als warp-to-grid <FILE.als> --tempo 134 --clips clips.json --json`
...
`<repo>/ableton-shared/bin/ableton als move-clip <FILE.als> --clip NAME --to-beat 7.0 --dur-s 7.5 --bpm 134 [--commit] --json`

## Snap several clips at once
Manifest `snaps.json`: `{"clip_name": {"beat": 7.0, "dur_s": 7.5, "bpm": 134}, ...}`,
then `<repo>/ableton-shared/bin/ableton als snap <FILE.als> --manifest snaps.json [--commit] --json`.
